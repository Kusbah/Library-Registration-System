...
public class Library extends Books {
    ...
    public Books searchByNumber(int number) {
        for (Books book : books) {
            if (book.getNo() == number) { // Assuming Books class has getNumber() method
                return book;
            }
        }
        return null;
    ...
    public Students getStudentById(int id) {
        for (Students student : students) {
            if (student.getID() == id) { // Assuming Students class has getId() method
                return student;
            }
        }
        return null;
    ...
    public boolean isBookAvailable(Books book) {
        for (Loan loan : loans) {
            if (loan.getBook().equals(book) && loan.isOverdue()) { // Assuming Loan has isReturned() method
                return false;
            }
        }
        return true;
    ...
    // Getters for books, students, and loans
